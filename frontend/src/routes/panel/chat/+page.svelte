<script lang="ts">
    ...
    onMount(async () => {
        try {
            const history = await getGeneralChatHistory();
            if (history.length > 0) {
                ...
                messages = [
                    {
                        role: "assistant",
                        content:
                            "Hello! I'm your LearningBuddy AI. Ask me anything about your uploaded sources -- lectures, documents, recordings, and more.",
                    },
                ];
            }
        } catch {
    ...
    async function send() {
        ...
        try {
            const fullResponse = await streamGeneralChat(
                userMessage,
                (chunk: string) => {
    ...
    async function handleClear() {
        if (!confirm("Clear all general chat history?")) return;
        try {
    ...
    <div class="flex flex-col h-[calc(100vh-6rem)] max-w-4xl mx-auto w-full">
        ...
        >
            <div class="flex items-center gap-3">
                <div
                    class="p-2 rounded-lg bg-brand-accent/10 text-brand-accent"
                >
        ...
        {#if chatMode === "text"}
            ...
            >
                <input
                    type="text"
                    bind:value={input}
                    placeholder="Ask about any of your sources..."
        ...
        {:else}
            <!-- Voice Chat (no sourceId = general mode) -->
            <div
